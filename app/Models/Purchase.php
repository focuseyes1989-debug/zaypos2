...
final class Purchase extends BaseModel
{
    ...
    public function __construct(array $data)
    {
        ...
        $this->supplierId = $this->requiredPositiveInteger(
        ...
        $this->purchaseNumber = $this->requiredString(
            $data['purchase_number'] ?? null,
            'Purchase number',
            100
        );

        $this->supplierInvoiceNumber = $this->nullableString(
            $data['supplier_invoice_number'] ?? null,
            100
        );

        $this->purchaseDate = $this->requiredDate(
        ...
        $this->receivedAt = $this->nullableDateTime(
            $data['received_at'] ?? null
        );

        $this->receivedBy = $this->nullablePositiveInteger(
    ...
    public function receivedBy(): ?int
    {
    ...
    public function toArray(): array
    {
        return [
            ...
            'received_at' => $this->receivedAt?->format(
                'Y-m-d H:i:s'
            ),
    ...
    private function validateStatus(mixed $value): string
    {
        ...
        if (
            ...
            throw new InvalidArgumentException(
                'Purchase status must be draft, received or cancelled.'
            );
        }
    ...
    private function validatePaymentStatus(
        ...
        if (!is_string($value)) {
            throw new InvalidArgumentException(
                'Purchase payment status is invalid.'
            );
        }
        ...
        if (
            ...
            throw new InvalidArgumentException(
                'Purchase payment status must be unpaid, partial or paid.'
            );
        }
